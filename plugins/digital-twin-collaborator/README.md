...
suite. This one is a pure skill — no MCP server, no network calls, nothing leaves the machine.
...
Everything here is free. No server, no licence key, no gated tool — a skill file and three
reference files that load into the conversation. The suite's paid tiers apply to plugins that
ship an MCP server; this is not one of them.
